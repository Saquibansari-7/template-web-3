...
export default function Countdown({ content }: { content: WebsiteContent }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const d = Math.floor(Math.max(distance, 0) / (1000 * 60 * 60 * 24));
      const h = Math.floor((Math.max(distance, 0) % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
      const m = Math.floor((Math.max(distance, 0) % (1000 * 60 * 60)) / (1000 * 60));
      const s = Math.floor((Math.max(distance, 0) % (1000 * 60)) / 1000);
      setTime({
      ...
      <div className="container mx-auto px-6 text-center z-10">
        <div className="max-w-2xl mx-auto mb-16">
          ...
          <p className="font-serif text-xl italic text-gray-700 leading-relaxed">
            ...
            We are so excited to celebrate our love with the people who mean the most to us. Join us for a day filled with laughter, love, and happily ever afters.
          </p>
        </div>
